/*
 * Copyright 2006-2020 smartics, Kronseder & Reiner GmbH
 *
...
public final class MavenScmRevisionNumberFetcher
  ...
  private static final Log LOG =
      LogFactory.getLog(MavenScmRevisionNumberFetcher.class);
   ...
   */
  private abstract static class AbstractScmResultWrapper
  ...
  public Revision fetchLatestRevisionNumber() throws ScmException {
    ...
    if (remoteVersion != null && "git".equals(repository.getProvider())) {
      final Revision revision =
          scmAccessInfo.fetchRemoteGitVersion(repository, remoteVersion);
      return revision;
    } else {
  ...
  private ScmResultWrapper execute(final ScmFileSet fileSet,
    ...
    if (remoteVersion != null) {
      ...
      result = createScmDiffResultWrapper(
  ...
  private ScmResultWrapper createScmDiffResultWrapper(
      final DiffScmResult diff) {
  ...
  private LocallyModifiedInfo createLocallyModifiedInfo(
    ...
    return new LocallyModifiedInfo(locallyModified,
        locallyModified ? toString(createSortedFiles(changedFiles)) : null);
  }

  private static Set<ScmFile> createSortedFiles(
