...
public class MavenScmRevisionNumberFetcher implements RevisionNumberFetcher
{
   ...
   * @return the largest revision number from the set..
   */
  private Revision findEndVersion(final ChangeLogSet changeLogSet)
  {
    ...
    final List changeSets = changeLogSet.getChangeSets();
    if (!changeSets.isEmpty())
    {
      final int lastIndex = changeSets.size() - 1;
      for (int index = lastIndex; index >= 0; index--)
      {
        final ChangeSet set = (ChangeSet) changeSets.get(lastIndex);
        final List changeFiles = set.getFiles();
        if (!changeFiles.isEmpty())
        {
          final ChangeFile file = (ChangeFile) changeFiles.get(0);
